...
def default_firefox_profile_path
  ...
  unless File.exist?(profile_ini)
    warn "profile.ini not found"
    exit(1)
  end
  ...
  if matches.nil?
    warn "default profile not found"
    exit(1)
  end
  ...
  File.expand_path(path, is_relative ? FIREFOX_BASE_DIR : nil)
end
