...
function! x2a#drawing#Underline(delimiter) abort
...
function! x2a#drawing#Line(delimiter) abort
  ...
  call x2a#drawing#Underline(a:delimiter)
...
function! x2a#drawing#AppendTrailer(delimiter) abort
  if matchstr(getline(line('.')), '\S') ># ''
...
" prompted trailer
function! x2a#drawing#InputTrailer() abort
  let l:delimiter = input('Line character: ')
  if l:delimiter ># ''
    call x2a#drawing#AppendTrailer(l:delimiter[0])
  endif
endfunction

function! x2a#drawing#RightJustify() abort
  if matchstr(getline(line('.')), '\S') ># ''
...
function! x2a#drawing#InsertLeader(delimiter) abort
  if matchstr(getline(line('.')), '\S') ># ''
    ...
    call x2a#drawing#AppendTrailer(a:delimiter)
...
" prompted leader
function! x2a#drawing#InputLeader() abort
  let l:delimiter = input('Line character: ')
  if l:delimiter ># ''
    if l:delimiter ==# ' '
      call x2a#drawing#RightJustify()
    else
      call x2a#drawing#InsertLeader(l:delimiter[0])
    endif
  endif
endfunction
...
function! x2a#drawing#BoxIn() abort
...
function! x2a#drawing#SectionBox(border) abort
  " remove indentation
