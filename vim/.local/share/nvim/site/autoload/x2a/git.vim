function! x2a#git#permalink(raw, line1, count) abort
  if !empty(getbufvar(bufnr('%'), '&buftype'))
    call x2a#utils#echo#Warning('Buffer is not a file')
    return
  endif
  ...
  if !v:shell_error
    ...
    call x2a#utils#echo#Warning(l:result)
  endif
endfunction
...
function! x2a#git#browse(raw, line1, count) abort
  ...
  if empty(l:browser)
    if executable('open')
      ...
      call x2a#utils#echo#Warning('Unable to resolve web browser or URL handler')
      return
    endif
  endif

  if !executable(l:browser)
    call x2a#utils#echo#Warning('Web browser or URL handler not executable: ' . l:browser)
    return
  endif
...
function! x2a#git#copy_permalink(raw, line1, count) abort
  ...
  if has('clipboard')
    ...
    call x2a#utils#echo#Warning('clipboard feature not available in Vim')
  endif
