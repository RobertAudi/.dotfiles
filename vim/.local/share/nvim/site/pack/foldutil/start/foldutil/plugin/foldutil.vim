...
if !exists('g:loaded_genutils') || g:loaded_genutils < 200
  echomsg 'foldutil: You need a newer version of genutils.vim plugin'
  finish
endif

let g:loaded_foldutil = 400
...
command! -range -nargs=? FoldShowMarks
      \ :let b:fuSearchFn = 's:PositionAtNextLine' |
      \ call <SID>FoldMatching(<line1>, <line2>, '!', <SID>GetMarkedLines(), <f-args>)
...
function! s:FoldMatching(fline, lline, bang, ...) abort " {{{
  call s:BeginFolding()
  ...
  if b:fuBgPattern ==# ''
    echohl Error
    echo 'No previous search pattern exists. Pass a search ' .
  ...
  if a:fline == a:lline && a:fline == line('.')
    ...
    " FIXME: Check if this is required (ie., check if vim already does it for us).
    if b:fuFline < 1
      let b:fuFline = 1
    endif
  ...
  if s:nFolds != 0
    ...
    echo 'Created: ' . s:nFolds . ' folds in: ' . (localtime() - l:stTime) . ' seconds'
  endif
endfunction " }}}
...
function! s:FoldShowRange(fline, lline) range " {{{
  ...
  if a:firstline != 1
    execute '1,' . a:firstline . '-1 fold'
  endif

  if a:lastline != line('$')
    execute a:lastline . '+1,$ fold'
  endif
endfunction " }}}
...
function! s:CreateFoldsInLoop() " {{{
  ...
  while 1
    try
      if !s:blockStarted
        let l:stLine = {b:fuSearchFn}(b:fuBgPattern, b:fuBgNegate)

        if l:stLine != 0 && l:stLine <= b:fuLline
          " Found begin pattern.
        ...
        let l:enLine = {b:fuSearchFn}(b:fuEnPattern, b:fuEnNegate)

        if l:enLine > b:fuLline
          let l:enLine = b:fuLline
        ...
        if l:enLine != 0 " Found the end of the block.
          if b:fuBlockMode != 1 && !l:lastBlock
            let l:enLine = l:enLine - 1
          endif

          if (l:enLine - s:blockStarted) > (b:fuContext + (!l:lastBlock * b:fuContext) - 1)
            call s:CreateFold(s:blockStarted + b:fuContext, l:enLine - (!l:lastBlock * b:fuContext))
          endif
      ...
      if line('.') > b:fuLline || l:curLine == line('.')
        break
      endif
  ...
  echo 'Folds created: ' . l:foldCount
...
function! FoldCreateFoldsStr(fline, lline) " {{{
  ...
  let l:inFold = 0
  let l:foldLevel = 0
  let l:prevFoldLevel = 0

  " We need a stack of start lines, as we need to take care of the nesting.
  let l:foldStLines = []
  let l:line = (a:fline > 0) ? a:fline : 1
  let l:lline = (a:lline ==# '$' || a:lline > line('$')) ? line('$') : a:lline

  while l:line <= l:lline
    let l:foldLevel = foldlevel(l:line)

    " If greater than previous fold level, that means we just entered into a
    "   new fold, so start the fold.
    if l:foldLevel > l:prevFoldLevel
      " Push this line.
      call add(l:foldStLines, l:line)
    elseif l:foldLevel < l:prevFoldLevel
      let l:stLine = l:foldStLines[-1] " Last element.

      " Strictly speaking, this is not possible.
      if l:stLine !=# ''
        call remove(l:foldStLines, len(l:foldStLines) - 1)

        let l:crFoldStr = l:crFoldStr . l:stLine . ',' . (l:line - 1) . "fold\n"
      endif
    endif

    let l:prevFoldLevel = l:foldLevel
    let l:line = l:line + 1
  endwhile
  ...
  for l:stLine in reverse(l:foldStLines)
    " Strictly speaking, this is not possible.
    if l:stLine !=# ''
      let l:crFoldStr = l:crFoldStr . l:stLine . ',' . (l:line - 1) . "fold\n"
    endif
...
function! s:SearchForPattern(pat, negate) abort
  let l:line = 0
...
function! s:PositionAtNextLine(lines, negate) abort
  if s:curLinesPattern ==# a:lines
    let l:lines = s:curLines
  else
    let l:lines = sort(split(a:lines, ','), 'genutils#CmpByNumber')
    let s:curLines = l:lines
  ...
  if a:negate
    ...
    while l:i <= l:lastLine && index(l:lines, l:i) != -1
      let l:i = l:i + 1
    endwhile
    ...
    for l:nextLine in l:lines
      if l:nextLine > l:curLine
        execute l:nextLine
...
function! s:PositionAtNextHiGroup(hiName, negate) abort
  ...
  while l:i <= l:lastLine
    try
      ...
      if l:lastCol != 1 " Blank line
        " We need to look through the entire line to determine whether the
        " highlight group exists or doesn't exist.
        let l:j = 1

        while l:j < l:lastCol
          let l:matches = l:matches || (synIDtrans(synID(l:i, l:j, 0)) == hlID(a:hiName))
          let l:j = l:j + 1
        endwhile
      endif

      if a:negate
        let l:matches = !l:matches
      endif
...
function! s:GetMarkedLines() abort
  let l:lines = ''
  let l:marks = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'
  let l:i = 0
  let l:max = strlen(l:marks)

  while l:i < l:max
    let l:line = line("'" . l:marks[l:i])
...
" the buffer.
function! s:SaveSetting(setting) abort
  ...
  execute 'let b:fuOrg{a:setting} = &l:' . a:setting

  call add(b:fuOrgSettings, a:setting)
endfunction

function! s:RestoreSettings() abort
  for l:nextSett in b:fuOrgSettings
    "let &l:{nextSett} = b:fuOrg{nextSett}

    execute 'let &l:' . l:nextSett . ' = b:fuOrg{nextSett}'

    unlet b:fuOrg{l:nextSett}
  endfor

  unlet b:fuOrgSettings
endfunction

function! s:BeginFolding() abort
  if !exists('b:fuSettStr')
    let b:fuSettStr = ''
